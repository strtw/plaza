...
- Only targets test users (users with `clerkId` starting with `'test_'`)
...
The simulation is controlled by an environment variable:
...
When disabled, the cron job still runs every 5 minutes but returns immediately (minimal compute cost).

### Safety Features

1. **Environment Check**: Only runs when `NODE_ENV !== 'production'`
2. **Feature Flag**: Requires `ENABLE_STATUS_SIMULATION=true` to actually run
3. **Test Users Only**: Only affects users with `clerkId` starting with `'test_'`
4. **User Limit**: Processes maximum of 10 users per run
5. **Error Handling**: Errors are logged but don't crash the cron job or affect other users

### Usage

1. Create test users using the "Create Mock Users" button in the app (creates users with `clerkId` starting with `'test_'`)
2. Set `ENABLE_STATUS_SIMULATION=true` in Railway environment variables
3. Wait 5 minutes for the first simulation run
4. Check the app to see statuses being created/updated/cleared automatically
5. When done testing, remove or disable the `ENABLE_STATUS_SIMULATION` env var

### Logs

The service logs all actions:
- `[DevService] Status simulation started`
- `[DevService] Simulating status changes for X test users`
...
- **Default**: Feature is OFF (cron runs but returns immediately - minimal cost)
